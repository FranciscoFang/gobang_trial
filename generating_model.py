...
while i<epochs:
    ...
    while check_win_all(board) == 0:
        ...
        while check_win_all(board) == 0 and black_play == True:
            if i<=10 : #first x random games
                return_board_black = machine_1_random(board)
            else:
                return_board_black = machine_1(board, model1)
            if return_board_black == [] or ''.join(str(return_board_black)).find('1') == -1 : print("检查是否在下子")
            allgame_black += [divide_board(board)]
            allaction_black += [return_board_black]
            board = combine_board(board, return_board_black)
            steps_count += 1
            black_play = False
            break
        if check_win_all(board) != 0: break
        if steps_count >= 225: break
        print ("到白方了")
        print (steps_count)
        while check_win_all(board) == 0 and black_play == False:
            if i<=10 : #first x random games
                return_board_white = machine_2_random(board)
            else:
                return_board_white = machine_2(board, model1)
            allgame_white += [divide_board(board)]
            allaction_white += [return_board_white]
    ...
    if check_win_all(board) == 1:
        ...
        black_wins += 1
        print ("黑赢+1", "黑AI获胜次数：", black_wins, "白AI获胜次数：", white_wins)
    ...
    if check_win_all(board) == -1:
        ...
        white_wins += 1
        print ("白赢+1", "黑AI获胜次数：", black_wins, "白AI获胜次数：", white_wins)
        ML_fit_2(np.asarray(sum_allgame_white), np.asarray(sum_allaction_white))
    if  check_win_all(board) == 0 and steps_count == 225:
        draw_game += 1
        print ("平局+1")
    if i%50 == 0:
        print ("局目总数：", i, "黑AI获胜次数：", black_wins, "白AI获胜次数：", white_wins, "平局次数：", draw_game, "", file=logs)
logs.close()
